-- ============================================================================
-- Fix bet_propose to prevent duplicate initial deposits
-- Only create initial deposit if user truly has no ledger entries
-- ============================================================================
...
DECLARE 
  new_id BIGINT;
  tx BIGINT;
  available_balance_cents BIGINT := 0;
  wallet_balance_cents BIGINT := 0;
...
BEGIN
  ...
  -- Check if proposer has ANY ledger entries (not just AVAILABLE)
  ...
  -- Only create initial deposit if user has NO ledger entries at all
  -- AND has a wallet_balance > 0
  IF ledger_count = 0 THEN
...
COMMENT ON FUNCTION bet_propose IS 'Propose a new bet and hold proposer funds immediately. Only creates initial deposit if user has no ledger entries.';
