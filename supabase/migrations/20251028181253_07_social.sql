...
-- 1. users(user_id)  -> used by all social tables (friends, circles, etc.)
-- 2. core.bets_settlements  -> used (read-only) for head-to-head stats & leaderboards

-- What other modules depend on this one:
-- 1. Direct Bets / Order Book -> reference circles(circle_id) for private bets
-- 2. Comments / Discussions   -> check friends & circle_members for visibility
-- 3. Reputation / Profiles    -> read head_to_head_stats & leaderboards for stats
...
SELECT
    ...
    SUM(CASE 
            WHEN b.winner_id = b.proposer_id THEN b.amount_cents
            WHEN b.winner_id = b.acceptor_id THEN -b.amount_cents
            ELSE 0 END)                           AS pnl_user_a
FROM bets_settlements b
GROUP BY
...
CREATE TABLE IF NOT EXISTS leaderboards_user_daily (
