-- Update trigger to check dispute_notes instead of outcome_notes for disputes
    ...
    CASE OLD.status
      ...
      WHEN 'ACTIVE' THEN
        ...
        IF NEW.status = 'DISPUTED' THEN
          -- Updated to check dispute_notes instead of outcome_notes
          IF NEW.dispute_notes IS NULL OR length(trim(NEW.dispute_notes)) = 0 THEN
            RAISE EXCEPTION 'Dispute notes required to open a dispute';
          END IF;
