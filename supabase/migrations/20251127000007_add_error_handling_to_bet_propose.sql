-- ============================================================================
-- Add better error handling and ensure ledger inserts work
-- ============================================================================

-- Update bet_propose with explicit error handling
...
DECLARE 
  ...
  has_ledger_entries BOOLEAN := FALSE;
  v_error_msg TEXT;
BEGIN
  ...
  -- Now hold the funds - wrap in exception handling
  BEGIN
    ...
    IF NOT has_ledger_entries OR available_balance_cents = 0 THEN
      ...
      IF wallet_balance_cents > 0 THEN
        ...
        IF tx IS NULL THEN
          RAISE EXCEPTION 'Failed to create deposit transaction';
        END IF;
    ...
    IF tx IS NULL THEN
      RAISE EXCEPTION 'Failed to create HOLD transaction';
    END IF;
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    -- Rollback the bet creation by raising an exception
    RAISE EXCEPTION 'Error holding funds for bet: %', v_error_msg;
  END;
